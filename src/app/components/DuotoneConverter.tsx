...
export type DuotoneConverterProps = {
    shadowHex: string;         // contoh: "#145527"
    highlightHex: string;      // contoh: "#F784C5"
    maxSizeMB?: number;        // default 10
    className?: string;
    onChange?: (dataUrl: string | null) => void; // terpanggil setiap ada hasil
    showUrlInput?: boolean;    // default true
};

const clamp01 = (x: number) => Math.min(1, Math.max(0, x));
const lerp = (a: number, b: number, t: number) => a + (b - a) * t;
// S-curve kecil agar kontras lebih “film-like”
...
function applyDuotoneToCanvas(
    ...
    for (let i = 0; i < d.length; i += 4) {
        ...
        let L = 0.2126 * r + 0.7152 * g + 0.0722 * b; // luminansi sRGB
        L = contrast(L, 0.15);
...
export default function DuotoneConverter({
    ...
    const fromUrl = useCallback((url: string) => {
        try { new URL(url); } catch { return alert('URL tidak valid.'); }
        setLoading(true);
        const img = new Image();
        img.crossOrigin = 'anonymous';
        img.onload = () => { requestAnimationFrame(() => { processImage(img); setLoading(false); }); };
        img.onerror = () => { setLoading(false); alert('Gagal memuat URL (CORS?). Coba unggah file.'); };
        img.src = url;
    }, [processImage]);
        ...
        <section className={className}>
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-gray-200/50 overflow-hidden mb-8">
                <div className="p-8">
                    ...
                    <div
                        {...getRootProps()}
                        className={`border-2 border-dashed rounded-xl px-8 py-30 text-center transition-all cursor-pointer
                        ...
                        <div className="mt-6">
                            ...
                            <div className="flex gap-3">
                                <input
                                    ...
                                    className="text-gray-500 flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none"
